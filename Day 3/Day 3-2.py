...
doIndexes = [i for i in range(len(input)) if input.startswith("do()", i)]
dontIndexes = [i for i in range(len(input)) if input.startswith("don't()", i)]


while "don't()" in input:
    deleteSegment = r"don't\(\).*?(do\(\)|$)"
    input = re.sub(deleteSegment, "", input)
